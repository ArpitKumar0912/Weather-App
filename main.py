...
def data_get():
       city = city_name.get()
       data= requests.get("https://api.openweathermap.org/data/2.5/weather?q="+city+"&appid=2870ece9dfbf1b966340ca1c65369db2").json()
       w_label1.configure(text=data["weather"][0]["main"])
       wb_label1.configure(text=data["weather"][0]["description"])
       temp_label1.configure(text=str(int(data["main"]["temp"]-273.15)))
       per_label1.configure(text=data["main"]["pressure"])

win = Tk()
win.title("My_Weather")
